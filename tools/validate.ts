...
type Demand = Item & { id: string; patron: string; axis: string; polarity: string; min_enemies: number; text: string; condition: string; reward: { favor?: number } };
type Enemy = Item & {
  id: string;
...
function schemaFailure(item: Item, kind: string): boolean {
  ...
  // 조건은 **하나**다 — 갈림은 한 신의 두 단이 아니라 경쟁하는 두 신이다 (P-59)
  if (kind === "demand") {
...
function demandFailure(demand: Demand): boolean {
  ...
  const parsed = demand.condition.match(/^([a-z_]+) (>=|<=|>|==) (\d+)$/);
...
function failureFor(item: Item, cards: Card[], enemies: Enemy[]): FailureKey | undefined {
...
export function validateItems(items: Item[], basePool: Card[] = []): { accepted: Item[]; rejected: { id: string; failure: FailureKey }[]; pass_rate: number; by_pairing: Record<string, number>; failure_breakdown: Partial<Record<FailureKey, number>>; pools: Record<string, PoolStat>; passive_coverage: string[]; unplaced_groups: string[]; grace_coverage: string[] } {
  ...
  for (const item of items) {
    const failure = failureFor(item, cards, enemies);
    if (failure) {
