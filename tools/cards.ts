 ...
 * 카드는 회차마다 는다 — 129장이 한 회차에 149장이 됐다.
 *
...
type Card = { id: string; name: string; patron?: string; patron_pair?: string[]; cost: number; target: string; effects: Effect[]; tags: string[]; trigger?: string; reach?: string; tier?: number };

const cards = cardData as Card[];
const godNames: Record<string, string> = { zeus: "제우스", poseidon: "포세이돈", athena: "아테나", ares: "아레스", artemis: "아르테미스" };
const tokenNames: Record<string, string> = { shock: "감전", displace: "밀려남", soaked: "침수", bulwark: "보루", deflect: "반사", bleed: "출혈", frenzy: "광란", mark: "표식", crit: "치명", thorns: "가시" };
