  ...
  return (
    <div className={`stats-stagebar${compact ? " compact" : ""}`}>
      {Object.entries(stageLabel).map(([stage, label]) => {
        ...
        return (
          <div key={stage} className="stats-stageseg" style={{ flexGrow: Math.max(value, 0.001) }} title={`${label} ${pct(value)}`}>
    ...
    <div className="stats-page">
      ...
      <section>
        ...
        <p>스냅샷은 노드 사이에 찍힌다 — 전투 안에서 호의는 움직이지 않는다. 분포는 이봉이다:
          가운데(+2 · 0 · −3 · −4)는 카드·감쇠 드리프트, 바깥(±12 이상)은 과업의 보상·비용이고 그 사이가 비어 있다.
