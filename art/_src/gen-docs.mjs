...
add({
  ...
  ref: "§2.5",
  spec: "PNG 알파, **생성 원본 해상도 유지 — 축소하지 않는다.** 화면 크기는 `ui/style.css`가 정한다: 경로 화면 36×48, 결과 화면 18×24. `.map-node.here`의 **가운데**에 얹힌다 — 테두리 색만으로는 **병사가 어디까지 올라왔는지**가 안 읽힌다.",
  gen: "1024×1024 이상 — 정사각으로 뽑는다",
  convert:
