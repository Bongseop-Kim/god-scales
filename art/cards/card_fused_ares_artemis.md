# `card_fused_ares_artemis.webp` — 피의 사냥 (아레스 + 아르테미스)
...
```
magick art/_src/cards/card_fused_ares_artemis.png -gravity center -crop 1365x1024+0+0 +repage \
  -filter Lanczos -resize 512x384! -quality 88 art/cards/card_fused_ares_artemis.webp
```
...
```text
A single fused emblem filling the frame: a single dark red arrow driving diagonally through the frame with a pale amethyst trail breaking behind it. Treat it like a game ability emblem: one instantly readable shape built from a few decisive angular strokes with hard corners, thick and heavy, no thin lines, no scenery, no architecture, no clouds.
