...
magick art/_src/cards/card_fused_poseidon_ares.png -gravity center -crop 1365x1024+0+0 +repage \
  -filter Lanczos -resize 320x240! -quality 80 art/cards/card_fused_poseidon_ares.webp
```
...
```text
A single fused emblem filling the frame: one heavy curl of dark teal water with a dark red current threading through its interior, the two colours braided but not blended. Treat it like a game ability emblem: one instantly readable shape built from a few decisive angular strokes with hard corners, thick and heavy, no thin lines, no scenery, no architecture, no clouds.
