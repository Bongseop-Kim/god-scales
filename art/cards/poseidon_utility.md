# `poseidon_utility.webp` — 포세이돈 · 유틸리티 태그
...
```
magick art/_src/cards/poseidon_utility.png -gravity center -crop 1365x1024+0+0 +repage \
  -filter Lanczos -resize 512x384! -quality 88 art/cards/poseidon_utility.webp
```
...
```text
A single thick broken whirlpool ring filling the frame, drawn as a few decisive angular strokes with hard corners, its center completely empty and dark so the shape reads as a hollow circle with a gap in it. Nothing inside the ring, nothing behind it. No orb, no sphere, no filled disc, no scenery, no architecture, no clouds.
