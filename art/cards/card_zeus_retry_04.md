# `card_zeus_retry_04.webp` — 감전 보호막 (제우스)
...
```
magick art/_src/cards/card_zeus_retry_04.png -gravity center -crop 1365x1024+0+0 +repage \
  -filter Lanczos -resize 320x240! -quality 80 art/cards/card_zeus_retry_04.webp
```
...
```text
Create a broad squat barrier across the lower half derived literally from the Korean card title “감전 보호막”. Use the title’s concrete object or force as the main motif, and make its silhouette distinct from every other zeus card rather than varying only the size or angle.
