# `card_zeus_retry_02.webp` — 급속 방전 (제우스)
...
```
magick art/_src/cards/card_zeus_retry_02.png -gravity center -crop 1365x1024+0+0 +repage \
  -filter Lanczos -resize 320x240! -quality 80 art/cards/card_zeus_retry_02.webp
```
...
```text
Create a forceful diagonal weapon or impact derived literally from the Korean card title “급속 방전”. Use the title’s concrete object or force as the main motif, and make its silhouette distinct from every other zeus card rather than varying only the size or angle.
