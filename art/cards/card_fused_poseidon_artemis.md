# `card_fused_poseidon_artemis.webp` — 달빛 파도 (포세이돈 + 아르테미스)

[P-32](../../plans/32-art.md) §3 · [원본 규칙](../README.md) · **상태 완료 · 512×384 — 아테나·아르테미스는 `godColors`와 색상 자체가 달랐다. §3 표의 hex가 정본이고 그것으로 그렸다**
...
```
magick art/_src/cards/card_fused_poseidon_artemis.png -gravity center -crop 1365x1024+0+0 +repage \
  -filter Lanczos -resize 320x240! -quality 80 art/cards/card_fused_poseidon_artemis.webp
```
...
```text
A single fused emblem filling the frame: one broad teal wave with a pale amethyst crescent low behind it, the crescent's light catching only the wave's upper edge. Treat it like a game ability emblem: one instantly readable shape built from a few decisive angular strokes with hard corners, thick and heavy, no thin lines, no scenery, no architecture, no clouds.
