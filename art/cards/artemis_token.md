# `artemis_token.webp` — 아르테미스 · 토큰 태그
...
```
magick art/_src/cards/artemis_token.png -gravity center -crop 1365x1024+0+0 +repage \
  -quality 88 art/cards/artemis_token.webp
```
...
```text
Five or six small arrow fletchings hanging scattered in mid-air across the frame at different heights, none of them touching, each one a simple hard-edged fragment, the whole spread reading as a scatter rather than a pile or a heap. No stack, no pile, no coins, no altar, no scenery, no architecture.
