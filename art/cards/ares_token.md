...
```
magick art/_src/cards/ares_token.png -gravity center -crop 1365x1024+0+0 +repage \
  -filter Lanczos -resize 512x384! -quality 88 art/cards/ares_token.webp
```
...
```text
Five or six small blood droplets hanging scattered in mid-air across the frame at different heights, none of them touching, each one a simple hard-edged fragment, the whole spread reading as a scatter rather than a pile or a heap. No stack, no pile, no coins, no altar, no scenery, no architecture.
