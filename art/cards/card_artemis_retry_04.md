# `card_artemis_retry_04.webp` — 달의 화살비 (아르테미스)
...
```
magick art/_src/cards/card_artemis_retry_04.png -gravity center -crop 1365x1024+0+0 +repage \
  -filter Lanczos -resize 320x240! -quality 80 art/cards/card_artemis_retry_04.webp
```
...
```text
Create a forceful diagonal weapon or impact derived literally from the Korean card title “달의 화살비”. Use the title’s concrete object or force as the main motif, and make its silhouette distinct from every other artemis card rather than varying only the size or angle.
