# `card_artemis_retry_01.webp` — 사냥감 표식 (아르테미스)
...
```
magick art/_src/cards/card_artemis_retry_01.png -gravity center -crop 1365x1024+0+0 +repage \
  -filter Lanczos -resize 320x240! -quality 80 art/cards/card_artemis_retry_01.webp
```
...
```text
Create a forceful diagonal weapon or impact derived literally from the Korean card title “사냥감 표식”. Use the title’s concrete object or force as the main motif, and make its silhouette distinct from every other artemis card rather than varying only the size or angle.
