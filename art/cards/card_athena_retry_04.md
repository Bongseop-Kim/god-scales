# `card_athena_retry_04.webp` — 창벽 반격 (아테나)
...
```
magick art/_src/cards/card_athena_retry_04.png -gravity center -crop 1365x1024+0+0 +repage \
  -filter Lanczos -resize 320x240! -quality 80 art/cards/card_athena_retry_04.webp
```
...
```text
Create a forceful diagonal weapon or impact derived literally from the Korean card title “창벽 반격”. Use the title’s concrete object or force as the main motif, and make its silhouette distinct from every other athena card rather than varying only the size or angle.
