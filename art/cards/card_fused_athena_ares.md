# `card_fused_athena_ares.webp` — 전쟁의 방패 (아테나 + 아레스)
...
```
magick art/_src/cards/card_fused_athena_ares.png -gravity center -crop 1365x1024+0+0 +repage \
  -filter Lanczos -resize 512x384! -quality 88 art/cards/card_fused_athena_ares.webp
```
...
```text
A single fused emblem filling the frame: an olive-bronze shield seen wide and squat with a dark red axe head buried deep in its face, split radiating from the wound. Treat it like a game ability emblem: one instantly readable shape built from a few decisive angular strokes with hard corners, thick and heavy, no thin lines, no scenery, no architecture, no clouds.
