# `card_athena_retry_03.webp` — 전략적 후퇴 (아테나)
...
```
magick art/_src/cards/card_athena_retry_03.png -gravity center -crop 1365x1024+0+0 +repage \
  -filter Lanczos -resize 320x240! -quality 80 art/cards/card_athena_retry_03.webp
```
...
```text
Create a broad squat barrier across the lower half derived literally from the Korean card title “전략적 후퇴”. Use the title’s concrete object or force as the main motif, and make its silhouette distinct from every other athena card rather than varying only the size or angle.
