# `card_fused_poseidon_athena.webp` — 해벽 (포세이돈 + 아테나)
...
```
magick art/_src/cards/card_fused_poseidon_athena.png -gravity center -crop 1365x1024+0+0 +repage \
  -filter Lanczos -resize 512x384! -quality 88 art/cards/card_fused_poseidon_athena.webp
```
...
```text
A single fused emblem filling the frame: a wall of standing teal water held in place across the lower half by an olive-bronze rampart braced against it. Treat it like a game ability emblem: one instantly readable shape built from a few decisive angular strokes with hard corners, thick and heavy, no thin lines, no scenery, no architecture, no clouds.
