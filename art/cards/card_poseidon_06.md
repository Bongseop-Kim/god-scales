# `card_poseidon_06.webp` — 밀쳐내기 (포세이돈)
...
```
magick art/_src/cards/card_poseidon_06.png -gravity center -crop 1365x1024+0+0 +repage \
  -filter Lanczos -resize 320x240! -quality 80 art/cards/card_poseidon_06.webp
```
...
```text
Create five or six separate marks scattered at different heights derived literally from the Korean card title “밀쳐내기”. Use the title’s concrete object or force as the main motif, and make its silhouette distinct from every other poseidon card rather than varying only the size or angle.
