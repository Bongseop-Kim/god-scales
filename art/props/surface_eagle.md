# `surface_eagle.png` — 독수리

[P-32](../../plans/32-art.md) §2 프롭 · [원본 규칙](../README.md) · **상태 원본 해상도 그대로 · 안 늦었다**

**파일** PNG 알파. `surface_eagle_idle_1.png`~`surface_eagle_idle_4.png`는 각각 `896×1024`, `surface_eagle.png`는 4프레임 가로 스트립 `3584×1024`다.
...
**기존 원본 변환 기록(참고 전용)** — 현재 원본은 `art/_src/props/surface_eagle-base.png`, 재현 가능한 run은 `art/_src/sprite-runs/surface_eagle/`에 있다.

```
magick art/_src/props/surface_eagle.png -alpha on -fuzz 35% -transparent '#00ff00' \
  -trim +repage art/props/surface_eagle.png
```
