...
```
magick art/_src/gods/poseidon.png -gravity center -crop 2:3 +repage \
  -quality 90 art/gods/poseidon.webp
```
...
Lighting and colour: exactly one hue of light in the image, a dark sea teal #2e7d8f, covering about one quarter of the frame. That single hue is the ONLY colour in the image and it BURNS out of near-total darkness — the whole rest of the frame is charcoal and deep navy near-black #11131a with coarse brush texture. #2e7d8f exactly, never neon, never white-hot, never a white core, no rainbow, no secondary colour, no complementary accent.
...
- **신 색은 §3 정본 하나뿐이다** — 포세이돈는 `#2e7d8f`. `ui/style.css:2` CSS 변수 쪽이 정본이고 `ui/app.tsx:37` `godColors`는 범례에서만 쓰인다
